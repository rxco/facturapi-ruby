  ...
  class FacturapiObject < Hash
    ...
    def inspect
      if self.respond_to? :each
        if self.class.class_name != "FacturapiObject"
          self.to_s
        else
          self.to_a.map{|x| x[1] }
        end
    ...
    def load_from_enumerable(k,v)
      if v.respond_to? :each and !v.instance_of?(FacturapiObject)
        v = Facturapi::Util.convert_to_facturapi_object(k,v)
      end

      if self.instance_of?(FacturapiObject)
        self[k] = v
      else
